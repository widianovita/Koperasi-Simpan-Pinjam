...
<html lang="en">
  <head>
    ...
    <title>Edit Data Simpan | Koperasi</title>
  </head>
  <body>
    ...
    <div class="row no-gutters mt-5">
      ...
      <div class="col-md-10 p-5 pt-4">
        <h3><i class="fas fa-chalkboard-teacher"></i> Simpan</h3><hr>

        <div class="form-mhs mt-2">
          <h5>Edit Data Simpan</h5>

          <?php
          include '../koneksi.php';

          $no = $_GET['no'];
          $data = mysqli_query($con,"select * from tbl_simpan where no_simpan='$no'");
          while($d = mysqli_fetch_array($data)){ 
          ?>

          <form method="post" action="update.php" class="tbl mt-4">
            <div class="form-group row">
              <label for="colFormLabel" class="col-sm-2 col-form-label">No Simpan</label>
              <div class="col-sm-10">
                <input type="text" class="form-control" id="colFormLabel" placeholder="" name="no" value="<?php echo $d['no_simpan'] ?>">
              </div>
            </div>
            <div class="form-group row">
              <label for="colFormLabel" class="col-sm-2 col-form-label">Tanggal</label>
              <div class="col-sm-10">
                <input type="date" class="form-control" id="colFormLabel" placeholder="" name="tanggal" value="<?php echo $d['tanggal'] ?>">
              </div>
            </div>
            <div class="form-group row">
              <label for="colFormLabel" class="col-sm-2 col-form-label">No Anggota</label>
              <div class="col-sm-10">
                <select class="form-control" name="no_anggota">
                  <?php 
                  include '../koneksi.php';
                  $no_anggota = mysqli_query($con,"select * from tbl_anggota");
                  while ($data_anggota = mysqli_fetch_array($no_anggota)) {
                    if($d['no_anggota']==$data_anggota['no_anggota']) {
                      $select = "selected";
                    }
                    else {
                      $select = "";
                    }
                    echo "<option $select>".$data_anggota['no_anggota']."</option>";
                  }
                  ?>
                </select>
              </div>
            ...
            <div class="form-group row">
              <label for="colFormLabel" class="col-sm-2 col-form-label">Jumlah Simpan</label>
              <div class="col-sm-10">
                <input type="text" class="form-control" id="colFormLabel" placeholder="" name="jml" value="<?php echo $d['jml_simpan'] ?>">
              </div>
            </div>
            <div class="form-group row">
              ...
              <div class="col-sm-10">
                <select class="form-control" name="kode">
                  <?php 
                  $kode = mysqli_query($con,"select * from tbl_kasir");
                  while ($k = mysqli_fetch_array($kode)) {
                    if($d['kode_kasir']==$k['kode_kasir']) {
                      $select = "selected";
                    }
                    else {
                      $select = "";
                    }
                    echo "<option $select>".$k['kode_kasir']."</option>";
                  }
                  ?>
                </select>
              </div>
